...
export function createMinesweeperBoard(state: any, gameId: string) {
  ...
  for (let row = 0; row < 5; row++) {
    ...
    for (let col = 0; col < 5; col++) {
      ...
      const flagged = state.flagged[row][col];
      const cell = state.board[row][col];
      ...
      actionRow.addComponents(
        new ButtonBuilder()
          .setCustomId(`ms_${gameId}_${row}_${col}`)
          .setLabel(label)
          .setStyle(style)
          .setDisabled(state.gameOver || revealed)
      );
    }
    rows.push(actionRow);
  }
...
export function createMinesweeperEmbed(state: any, isGameOver = false) {
  const embed = new EmbedBuilder()
    .setTitle("💣 MINESWEEPER")
    .setDescription("Click cells to reveal. Long-press/right-click to flag.")
    .setColor(0x5865F2);
  
  if (isGameOver) {
    if (state.won) {
      const time = Math.floor((state.endTime - state.startTime) / 1000);
      embed.setDescription(`🎉 You won! Time: ${time}s`);
      embed.setColor(0x57F287);
    } else {
      embed.setDescription("💥 Game Over! You hit a mine.");
